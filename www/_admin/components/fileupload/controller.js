var __filesForUpload__ = {};
var __hookedForms__ = {};

var recursiveSubmitForm = function(formEl) {
    var frmEl = $(formEl);

    var frmId = frmEl.attr("id");

    if (frmId in __hookedForms__) {
        return;
    }
    __hookedForms__[frmId] = frmId;

    var done = false;

    Object.keys(__filesForUpload__[frmId]).forEach(function(key,index) {
        var inputFiles = __filesForUpload__[frmId][key];
